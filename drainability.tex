...
We validate both results empirically in a slab-granularity allocator, observing a 238$\times$ differential in recycle rate between lifetime-mixed and lifetime-isolated routing under identical allocator logic and identical workload, differing only in allocation routing. This work does not introduce a new allocator; it characterizes the structural condition that determines whether coarse-grained reclamation produces bounded or unbounded memory retention in long-running systems.
\end{abstract}
...
We write $\rho(a)$ for the granule to which allocation $a$ is routed. Drainability (defined in Section~3) is a property of $\rho$ given the program's lifetime structure: the same program may produce drainable granules under one routing function and non-drainable granules under another. The central result of this paper is that reclamation success is determined by $\rho$, not by the allocator's reclamation policy.
...
For any allocator reclaiming memory at granularity $G$ under the non-relocating rule, a granule $g$ is reclaimable at its designated reclaim boundary $t_{\textit{reclaim}}(g)$ if and only if $g$ is drainable.
\end{theorem}
...
For slab allocators with size classes, let: $k$ denote a size class, $C_k$ the objects per slab (capacity for class $k$), $L_k(g)$ the live objects of class $k$ remaining in granule $g$ at its reclaim boundary, and $R_k(g)$ the retained slabs of class $k$ after the reclaim boundary of $g$.

A slab is reclaimable if and only if it contains zero live objects, so:
\[
R_k(g) \geq \lceil L_k(g) / C_k \rceil
\]

Any live object pins its entire slab. Total retained slabs by time $t$:
\[
R(t) = \sum_{g \text{ closed by } t} \sum_k R_k(g)
\]
...
\begin{proof}
Drainability implies $L_k(g) = 0$ for all size classes $k$ and all closed granules $g$, hence $R_k(g) = 0$ at close. Retained slabs therefore consist only of slabs in currently open granules and slabs in bounded recycling caches. Let $\textit{peak}_k$ be the maximum number of live objects of class $k$ in any single open granule. Then:
\[
...
\begin{theorem}
Let $m(t)$ be the number of granules closed by time $t$. If there exists $p > 0$ such that for at least a fraction $p$ of closed granules, at reclaim time there exists some size class $k$ with $L_k(g) \geq 1$, then:
\[
R(t) \geq p \cdot m(t)
\]
\end{theorem}
...
\begin{proof}
Each such granule contributes at least one retained slab (since $\lceil L_k(g)/C_k \rceil \geq 1$ when $L_k(g) \geq 1$). With at least $p \cdot m(t)$ such granules by time $t$, the bound follows.
\end{proof}
...
The allocator implementation is held constant across both experiments. The \emph{only} variable is the allocation routing function: whether allocations with different lifetime characteristics are routed to the same granules (mixing) or to separate granules (isolation).
...
\subsection{Lifetime Mixing (Drainability Violation)}

\textbf{Workload.} Long-lived session objects and short-lived request objects are routed into the same rotating granules.

\textbf{Violation condition:}
\[
\exists a_{\textit{session}}:\; \textit{lifetime}(a_{\textit{session}}) \not\subseteq \textit{lifecycle}(g_{\textit{request}})
\]
...
\midrule
Recycle rate & 0.28\% \\
Retained slabs & ${\sim}$493K \\
RSS growth rate & ${\sim}$9.7 MB / 1K requests \\
Empty slabs per close & ${\sim}$7 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Observed behavior.} Retained slabs grew at approximately 2.5 slabs per request, confirming the linear growth predicted by Theorem~3. The observed slope remained stable over the full measurement window, indicating steady-state sustained violation rather than transient burst effects.

\subsection{Lifetime Isolation (Drainability Satisfaction)}

\textbf{Workload.} Session objects are routed to a persistent granule; request objects are routed to rotating granules. No cross-lifetime mixing occurs.

\textbf{Satisfaction condition:}
\[
\forall g_{\textit{request}},\; \forall a \in \textit{allocs}(g_{\textit{request}}):\; t_{\textit{free}}(a) \leq t_{\textit{reclaim}}(g_{\textit{request}})
\]

\textbf{Results (20K requests):}
...
\midrule
Recycle rate & 66.5\% \\
Retained slabs & ${\sim}$2K \\
RSS plateau & ${\sim}$8 MB \\
Empty slabs per close & ${\sim}$266 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Observed behavior.} Retained slabs plateaued at approximately 2K regardless of total request volume, confirming the constant bound predicted by Theorem~2.
...
\noindent The recycle-rate differential is 238$\times$ (66.5\% / 0.28\%). The allocator implementation is identical in both experiments, under identical workload; only routing changed.

\subsection{Limitations}

This empirical validation uses a single workload (session + request objects), a single allocator, and a single size-class distribution. The 238$\times$ figure is specific to this configuration's parameters---particularly the ratio of session to request object lifetimes and the slab capacity. The theoretical results (Theorems~2 and 3) are general, but validating them across a broader range of workloads---multiple lifetime classes, real application traces, varying size-class distributions---remains future work.
...
\smallskip
\noindent Blackwell, D. (2026). Drainability: When Coarse-Grained Memory Reclamation Produces Bounded Retention. Technical Report.
