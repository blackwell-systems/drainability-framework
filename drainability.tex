...
\begin{theorem}
Let $m(t)$ be the number of granules closed by time $t$. If there exist $p > 0$ and $t_0 \geq 0$ such that for all $t \geq t_0$, at least a fraction $p$ of granules closed by time $t$ each contain some size class $k$ with $L_k(g, t) \geq 1$, then:
\[
R(t) \geq p \cdot m(t) \quad \text{for all } t \geq t_0.
\]
\end{theorem}

\begin{proof}
At any time $t \geq t_0$, each such granule contributes at least one retained slab (since $\lceil L_k(g, t)/C_k \rceil \geq 1$ when $L_k(g, t) \geq 1$). With at least $p \cdot m(t)$ such granules by time $t$, the bound follows.
\end{proof}
...
\bibitem{powers2019}
B.~Powers, D.~Emery, and E.~D.~Berger.
