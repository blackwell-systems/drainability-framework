...
\noindent where $R_k(g, t) = \lceil L_k(g, t) / C_k \rceil$ is the number of retained slabs of class $k$ in granule $g$ at time $t$.
...
\[
\exists a_{\textit{session}} \in \textit{allocs}(g_{\textit{request}}):\; t_{\textit{free}}(a_{\textit{session}}) > t_{\textit{reclaim}}(g_{\textit{request}})
\]
